...
export class ScriptValidator {
  ...
  validateStep(step: EngineStep, index: number): void {
    ...
    if (!['click', 'type', 'swipe', 'longpress', 'home', 'openApp', 'ai', 'checkText', 'visionClick', 'wait'].includes(step.type)) {
  ...
  private validateData(type: string, data: Record<string, any>, index: number): void {
    ...
    switch (type) {
      ...
      case 'checkText':
        if (!data.text) {
          throw new ScriptEngineError(ErrorCode.STEP_TYPE_INVALID, `步骤 ${index} 检测文本不能为空`, index)
        }
        break
      case 'visionClick':
        if (!data.target) {
          throw new ScriptEngineError(ErrorCode.STEP_TYPE_INVALID, `步骤 ${index} 目标描述不能为空`, index)
        }
        break
      case 'wait':
        if (typeof data.duration !== 'number' || data.duration <= 0) {
          throw new ScriptEngineError(ErrorCode.STEP_TYPE_INVALID, `步骤 ${index} 等待时长必须大于 0`, index)
        }
        break
    }
  }
