...
import { checkTextWithVLM } from '../services/vlm-service'
...
export async function checkScreenText(screenshotBase64: string, target: string, checkMode: 'text' | 'app'): Promise<ScreenCheckResult> {
  ...
  const rawResponse = await checkTextWithVLM(screenshotBase64, prompt)
  console.log(`[VLM] 原始响应: ${rawResponse}`)
  ...
  console.log(`[VLM] 检测结果: matched=${found}, desc="${description}"`)
